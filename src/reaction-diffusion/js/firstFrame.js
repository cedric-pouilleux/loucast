...
export function drawFirstFrame (type = InitialTextureTypes.CIRCLE) {
  ...
  const centerY = parameterValues.canvas.height / 2
...
function renderInitialDataToRenderTargets (initialData) {
  ...
  const texture = new THREE.DataTexture(initialData, parameterValues.canvas.width, parameterValues.canvas.height, THREE.RGBAFormat, THREE.FloatType)
  ...
  // Render the DataTexture into both of the render
  for (let i = 0; i < 2; i++) {
...
function getImagePixels (imageData, centerX, centerY) {
  ...
  return new Promise((resolve) => {
    ...
    bufferImage.addEventListener('load', () => {
      ...
      function scaleImageWithCanvas () {
        const widthRatio = parameterValues.canvas.width / bufferImage.width
        const heightRatio = parameterValues.canvas.height / bufferImage.height
        const bestFitRatio = Math.min(widthRatio, heightRatio)
        const scaledWidth = bufferImage.width * bestFitRatio
        const scaledHeight = bufferImage.height * bestFitRatio
...
function convertPixelsToTextureData () {
  const pixels = bufferCanvasCtx.getImageData(0, 0, parameterValues.canvas.width, parameterValues.canvas.height).data
