...
export default function setupMIDI () {
  WebMIDI.enable((error) => {
    ...
    // Akai LPD8 Wireless ---------------------------------------------------
    const lpd8 = WebMIDI.getInputByName('Akai LPD8 Wireless')
    ...
    const launchControlXL = WebMIDI.getInputByName('Launch Control XL')
...
function setupLaunchControlXL (launchControlXL) {
  ...
  launchControlXL.addListener('controlchange', 'all', (e) => {
    switch (e.controller.number) {
      ...
      case 33: // scale
        simulationUniforms.styleMapTransforms.value.x = e.value.map(0, 127, 0.1, 3.0)
